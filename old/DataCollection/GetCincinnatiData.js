...
exports.getCincinnatiData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Cincinnati,+OH&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Cincinnati-OH'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Cincinnati, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Cincinnati + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Cincinnati Jobs Not Working')})
  }).catch((err) => { console.log('Cincinnati Salary not working')});
}