...
exports.getAustinData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Austin,+TX&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Austin-TX'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Austin, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Austin + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Austin Jobs Not Working')})
  }).catch((err) => { console.log('Austin Salary not working')});
}