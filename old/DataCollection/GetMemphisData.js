...
exports.getMemphisData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Memphis,+TN&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Memphis-TN'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Memphis, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Memphis + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Memphis Jobs Not Working')})
  }).catch((err) => { console.log('Memphis Salary not working')});
}