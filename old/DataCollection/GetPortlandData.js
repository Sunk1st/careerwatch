...
exports.getPortlandData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Portland,+OR&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Portland-OR'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Portland, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Portland + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Portland Jobs Not Working')})
  }).catch((err) => { console.log('Portland Salary not working')});
}