...
exports.getNewOrleansData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=New+Orleans,+LA&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-New-Orleans-LA'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'NewOrleans, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.NewOrleans + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('NewOrleans Jobs Not Working')})
  }).catch((err) => { console.log('NewOrleans Salary not working')});
}