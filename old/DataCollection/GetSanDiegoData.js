...
exports.getSanDiegoData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=San+Diego,+CA&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-San-Diego-CA'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'SanDiego, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.SanDiego + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('SanDiego Jobs Not Working')})
  }).catch((err) => { console.log('SanDiego Salary not working')});
}