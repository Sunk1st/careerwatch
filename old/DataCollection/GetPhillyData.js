...
exports.getPhillyData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Philadelphia,+PA&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Philadelphia-PA'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Philly, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Philly + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Philly Jobs Not Working')})
  }).catch((err) => { console.log('Philly Salary not working')});
}