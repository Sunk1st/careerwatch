...
exports.getOklahomaCityData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Oklahoma+City,+OK&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Oklahoma-City-OK'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'OklahomaCity, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.OklahomaCity + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('OklahomaCity Jobs Not Working')})
  }).catch((err) => { console.log('OklahomaCity Salary not working')});
}