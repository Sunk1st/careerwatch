...
exports.getDallasData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Dallas,+TX&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Dallas-Fort-Worth-TX'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Dallas, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Dallas + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Dallas Jobs Not Working')})
  }).catch((err) => { console.log('Dallas Salary not working')});
}