...
exports.getMinneapolisData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Minneapolis,+MN&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Minneapolis-MN'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Minneapolis, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Minneapolis + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Minneapolis Jobs Not Working')})
  }).catch((err) => { console.log('Minneapolis Salary not working')});
}