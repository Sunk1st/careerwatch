...
exports.getMiamiData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Miami,+FL&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Miami-FL'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Miami, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Miami + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Miami Jobs Not Working')})
  }).catch((err) => { console.log('Miami Salary not working')});
}