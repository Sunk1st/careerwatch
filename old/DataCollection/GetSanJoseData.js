...
exports.getSanJoseData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=San+Jose,+CA&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-San-Jose-CA'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'SanJose, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.SanJose + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('SanJose Jobs Not Working')})
  }).catch((err) => { console.log('SanJose Salary not working')});
}