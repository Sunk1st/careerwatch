...
exports.getRaleighData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Raleigh,+NC&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Raleigh-NC'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Raleigh, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Raleigh + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Raleigh Jobs Not Working')})
  }).catch((err) => { console.log('Raleigh Salary not working')});
}