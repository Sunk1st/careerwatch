...
exports.getHoustonData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Houston,+TX&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Houston-TX'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Houston, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Houston + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Houston Jobs Not Working')})
  }).catch((err) => { console.log('Houston Salary not working')});
}