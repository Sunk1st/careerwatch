...
exports.getRiversideData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Riverside,+CA&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Riverside-CA'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Riverside, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Riverside + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Riverside Jobs Not Working')})
  }).catch((err) => { console.log('Riverside Salary not working')});
}