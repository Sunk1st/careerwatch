...
exports.getDetroitData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Detroit,+MI&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Detroit-MI'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Detroit, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Detroit + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Detroit Jobs Not Working')})
  }).catch((err) => { console.log('Detroit Salary not working')});
}