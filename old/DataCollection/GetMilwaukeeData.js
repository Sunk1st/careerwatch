...
exports.getMilwaukeeData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Milwaukee&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Milwaukee-WI'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Milwaukee, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Milwaukee + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Milwaukee Jobs Not Working')})
  }).catch((err) => { console.log('Milwaukee Salary not working')});
}