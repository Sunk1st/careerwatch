...
exports.getIndianapolisData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Indianapolis,+IN&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Indianapolis-IN'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Indianapolis, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Indianapolis + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Indianapolis Jobs Not Working')})
  }).catch((err) => { console.log('Indianapolis Salary not working')});
}