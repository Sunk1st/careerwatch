...
exports.getSaltLakeCityData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Salt+Lake+City,+UT&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Salt-Lake-City-UT'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'SaltLakeCity, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.SaltLakeCity + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('SaltLakeCity Jobs Not Working')})
  }).catch((err) => { console.log('SaltLakeCity Salary not working')});
}