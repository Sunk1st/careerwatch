...
exports.getOrlandoData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Orlando,+FL&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Orlando-FL'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Orlando, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Orlando + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Orlando Jobs Not Working')})
  }).catch((err) => { console.log('Orlando Salary not working')});
}