...
exports.getSeattleData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Seattle,+WA&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Seattle-WA'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Seattle, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Seattle + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Seattle Jobs Not Working')})
  }).catch((err) => { console.log('Seattle Salary not working')});
}