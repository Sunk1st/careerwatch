...
exports.getNashvilleData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Nashville,+TN&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Nashville-TN'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Nashville, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Nashville + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Nashville Jobs Not Working')})
  }).catch((err) => { console.log('Nashville Salary not working')});
}