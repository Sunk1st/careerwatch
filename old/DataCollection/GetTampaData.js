...
exports.getTampaData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Tampa,+FL&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Tampa-FL'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Tampa, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Tampa + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Tampa Jobs Not Working')})
  }).catch((err) => { console.log('Tampa Salary not working')});
}