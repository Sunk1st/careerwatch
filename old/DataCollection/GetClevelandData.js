...
exports.getClevelandData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Cleveland,+OH&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Cleveland-OH'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Cleveland, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Cleveland + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Cleveland Jobs Not Working')})
  }).catch((err) => { console.log('Cleveland Salary not working')});
}