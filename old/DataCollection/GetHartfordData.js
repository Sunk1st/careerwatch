...
exports.getHartfordData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Hartford,+CT&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Hartford-CT'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Hartford, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Hartford + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Hartford Jobs Not Working')})
  }).catch((err) => { console.log('Hartford Salary not working')});
}