...
exports.getCharlotteData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Charlotte,+NC&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Charlotte-NC'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Charlotte, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Charlotte + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Charlotte Jobs Not Working')})
  }).catch((err) => { console.log('Charlotte Salary not working')});
}