...
exports.getColumbusData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Columbus,+OH&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Columbus-OH'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Columbus, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Columbus + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Columbus Jobs Not Working')})
  }).catch((err) => { console.log('Columbus Salary not working')});
}