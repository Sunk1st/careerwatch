...
exports.getDCData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Washington,+DC&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Washington-DC'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'DC, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.DC + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('DC Jobs Not Working')})
  }).catch((err) => { console.log('DC Salary not working')});
}