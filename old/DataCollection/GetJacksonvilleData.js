...
exports.getJacksonvilleData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Jacksonville,+FL&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Jacksonville-FL'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Jacksonville, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Jacksonville + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Jacksonville Jobs Not Working')})
  }).catch((err) => { console.log('Jacksonville Salary not working')});
}