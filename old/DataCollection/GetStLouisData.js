...
exports.getStLouisData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=St.+Louis,+MO&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-St.-Louis-MO'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'StLouis, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.StLouis + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('StLouis Jobs Not Working')})
  }).catch((err) => { console.log('StLouis Salary not working')});
}