...
exports.getRichmondData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Richmond,+VA&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Richmond-VA'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Richmond, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Richmond + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Richmond Jobs Not Working')})
  }).catch((err) => { console.log('Richmond Salary not working')});
}