...
exports.getBaltimoreData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Baltimore,+MD&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Baltimore-MD'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Baltimore, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Baltimore + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Baltimore Jobs Not Working')})
  }).catch((err) => { console.log('Baltimore Salary not working')});
}