...
exports.getSacramentoData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Sacramento,+CA&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Sacramento-CA'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Sacramento, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Sacramento + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Sacramento Jobs Not Working')})
  }).catch((err) => { console.log('Sacramento Salary not working')});
}