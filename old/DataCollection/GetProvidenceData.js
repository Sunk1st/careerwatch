...
exports.getProvidenceData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Providence,+RI&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Providence-RI'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Providence, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Providence + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Providence Jobs Not Working')})
  }).catch((err) => { console.log('Providence Salary not working')});
}