...
exports.getDenverData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Denver,+CO&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Denver-CO'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Denver, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Denver + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Denver Jobs Not Working')})
  }).catch((err) => { console.log('Denver Salary not working')});
}