...
exports.getBostonData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Boston,+MA&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Boston-MA'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Boston, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Boston + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Boston Jobs Not Working')})
  }).catch((err) => { console.log('Boston Salary not working')});
}