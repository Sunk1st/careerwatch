...
exports.getKCData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Kansas+City,+MO&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Kansas-City-MO'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'KC, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.KansasCity + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('KC Jobs Not Working')})
  }).catch((err) => { console.log('KC Salary not working')});
}