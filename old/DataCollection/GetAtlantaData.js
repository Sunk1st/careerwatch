...
exports.getAtlantaData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Atlanta,+GA&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Atlanta-GA'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Atlanta, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Atlanta + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Atlanta Jobs Not Working')})
  }).catch((err) => { console.log('Atlanta Salary not working')});
}