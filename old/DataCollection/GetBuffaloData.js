...
exports.getBuffaloData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Buffalo,+NY&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Buffalo-NY'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Buffalo, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Buffalo + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Buffalo Jobs Not Working')})
  }).catch((err) => { console.log('Buffalo Salary not working')});
}