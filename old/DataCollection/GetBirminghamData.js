...
exports.getBirminghamData = function (salaryString, jobString, file) {
  let urlJob = 'https://www.indeed.com/jobs?q=' + jobString + '&l=Birmingham,+AL&jt=fulltime'
  let urlSalary = 'https://www.indeed.com/salaries/' + salaryString + '-Salaries,-Birmingham-AL'
  ...
  rp(salaries).then(($) => { 
    ...
    rp(jobs).then(($) => {
        ...
        let sendData = 'Birmingham, ' + finalResult + ',' + samples + ',' + totalJobs + ',' + city.sizeRank.Birmingham + '\n'
        fs.appendFile(file, sendData, function (err) {
          if (err) throw err;
      });
    }).catch((err) => { console.log('Birmingham Jobs Not Working')})
  }).catch((err) => { console.log('Birmingham Salary not working')});
}